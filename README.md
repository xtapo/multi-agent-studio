...
- **Provider abstraction** — OpenAI and Anthropic today, one interface for anything else.
...
| LLM | OpenAI · Anthropic (pluggable `LLMProvider`) |
| Tests | Vitest |
...
Then set `OPENAI_API_KEY`. Everything else has a working default.
...
Route handler  POST /api/workflows/:id/run      → returns runId immediately (202)
   ↓
Run orchestrator (background)  src/lib/orchestration/runner.ts
   ↓
...
src/
  ...
  lib/
    ...
    providers/             LLMProvider abstraction (OpenAI, Anthropic), pricing
